...
var TableAv = (function ($, eventDisp) {
	...
	TableAv.prototype.init = function (data) {
		this.$container.addClass('TableAv');

		this.createCalendar(data.years);
		
		this.$rooms = this.$container.find('.Av-rooms');
		
		this.createRooms(data.rooms);
	...
	TableAv.prototype.createYear = function (year, monStart, monEnd, tday, type) {
		...
		function createDays(d,monthes) {
			...
			if(self.calendarData[year][month]===undefined){
				self.calendarData[year][month] = {title:monthes[month-1]};
			}
			
			self.calendarData[year][month][date] = {title:weekday[numDay], date: date};
			return ceilHtml;
		}
		return yearHtml;
	};
	
	TableAv.prototype.createRooms = function (data) {
		for (var item in data) {			
			this.addRoom(data[item].name, data[item].id);
		}
	...
	TableAv.prototype.selectRangeChecks = function (left, right, top, bottom) {
		...
		for (var i=0; i < this.$allChecks.length; i++) {
			...
			if(
				...
				if(count===false){
					self.selectedPeriod.start = elId.slice(2);
				}else{
					self.selectedPeriod.end = elId.slice(2);
				}
	...
	TableAv.prototype._makeDraggable = function () {
      ...
      self._dragEl = { x: e.pageX, y: e.pageY, el: el, rect: rect, mouseOffset:{} };
    ...
    innerHTML += '<div class="Av-periodEnd">' + selectDaysHtml + selectMonsHtml + '</div>';
		...
		innerHTML +='<div class="radio"><label><input type="radio" name="Av-changeChecksType" value="+" checked> увеличить на</label></div>';
    innerHTML +='<input class="Av-changeChecksValue" type="number" value="0"></div>';
    innerHTML +='<div class="modal-footer"><button type="button" class="btn btn-default cancel">Отмена</button><button type="button" class="btn btn-primary save">Ok</button></div>';
  	innerHTML +='</div></div>';
	...
	TableAv.prototype.createGeneralActions = function () {
		...
		this.$btnSave = this.$container.find('.Av-GeneralSave');
		this.$btnCancel= this.$container.find('.Av-GeneralCancel');
	};
	...
	TableAv.prototype.changeValueChecks = function () {
		...
		for (var i in this.selectedChecks) {
			...
			initVal = +this.selectedChecks[i].html();
			if(type === '+'){
				newVal = initVal + val;
			}else if(type === '-'){
				newVal = initVal - val;
			...
			this.selectedChecks[i].html(newVal);			
		}
    ...
    self.$container.on('scroll', function(){
  		...
  		if(scrollLeft>1){
  			...
  			self.$rooms.css('position', 'fixed').css('top', pos.top-51-scrollTop);
  		}else{
  			self.$rooms.css('position', 'absolute').css('top', '');;
  		}
